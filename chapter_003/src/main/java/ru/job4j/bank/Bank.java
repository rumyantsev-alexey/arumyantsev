...
/*        User result = new User();
        ...
        Optional<User> result = users.keySet().stream().filter(k -> k.getPassport().equals(passport)).findFirst();
        if (result.isEmpty()) {
            throw new NoSuchUserException();
        }
        return result.get();
...
/*        Account result = new Account();
        ...
        Optional<Account> result = users.keySet().stream().flatMap(k -> users.get(k).stream()).filter(a -> a.getRequisites().equals(requisite)).findFirst();
        if (result.isEmpty()) {
            throw new NoSuchAccountException();
        }
        return result.get();
